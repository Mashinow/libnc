// Auto-generated from libnc.dll_pseudocode.c
// Root function: nc_new_vec_f32
// Included functions: 3

// ======================================================
// Function: nc_new_vec_f32 @ 0x66A19D80
// ======================================================

// local variable allocation has failed, the output may be wrong!
_QWORD *__fastcall nc_new_vec_f32(__int64 a1, __int64 a2, double _XMM2_8)
{
  __int64 v6; // r12
  _QWORD *v7; // rbx
  __int64 v8; // rdi
  __int64 *v9; // r14
  __int64 v10; // rdi
  _DWORD *v11; // rsi
  __int64 v12; // rax
  __int64 v13; // rax
  unsigned int v14; // edx
  __int64 v15; // rax
  __int64 v16; // rax
  int v17; // eax

  __asm { vmovd   r13d, xmm2 }
  if ( !a2 )
    sub_66A33D40("n >= 1", "libnc.c", 5404);
  v6 = *(_QWORD *)(a1 + 16);
  v7 = aligned_malloc(0x98ui64, 0x40ui64);
  if ( !v7 )
  {
    MEMORY[0x18] = v6;
    BUG();
  }
  memset(v7, 0, 0x98ui64);
  v7[15] = a2;
  v7[8] = a2;
  v8 = nc_type_size_table[0];
  *((_BYTE *)v7 + 73) = 1;
  *((_BYTE *)v7 + 72) = 1;
  v9 = *(__int64 **)(a1 + 16);
  v7[11] = v8;
  v10 = a2 * v8;
  v7[3] = v6;
  *((_DWORD *)v7 + 4) = 1;
  *((_DWORD *)v7 + 20) = 1;
  v11 = aligned_malloc(0x38ui64, 0x40ui64);
  v11[4] = 1;
  *((_QWORD *)v11 + 3) = a1;
  v12 = (*(__int64 (__fastcall **)(__int64, _DWORD *, __int64))(a1 + 48))(a1, v11 + 10, v10);
  *((_QWORD *)v11 + 4) = v12;
  if ( !v12 )
    nc_error("not enough device memory");
  v13 = *v9;
  *((_QWORD *)v11 + 6) = v10;
  v14 = _R13D;
  *(_QWORD *)(v13 + 8) = v11;
  *(_QWORD *)v11 = v13;
  v15 = *((_QWORD *)v11 + 5);
  *((_QWORD *)v11 + 1) = v9;
  *v9 = (__int64)v11;
  v7[7] = v15;
  v16 = *(_QWORD *)(v6 + 16);
  v7[5] = v11;
  *(_QWORD *)(v16 + 8) = v7;
  *v7 = v16;
  v7[1] = v6 + 16;
  v17 = *((_DWORD *)v7 + 12);
  *(_QWORD *)(v6 + 16) = v7;
  if ( v17 )
  {
    if ( v17 != 1 )
      abort();
    v14 = (_R13D + (HIWORD(_R13D) & 1) + 0x7FFF) >> 16;
  }
  nc_tensor_set_u32((__int64)v7, v14);
  return v7;
}

// ======================================================
// Function: sub_66A33D40 @ 0x66A33D40
// ======================================================

void __fastcall sub_66A33D40(const char *a1, const char *a2, int a3)
{
  size_t v5; // rax
  wchar_t *v6; // rsi
  size_t v7; // rax
  const wchar_t *v8; // rax
  wchar_t v9; // cx
  __int64 v10; // r9
  wchar_t *v11; // rbx
  wchar_t v12; // cx
  __int64 v13; // r9

  v5 = strlen(a1);
  v6 = (wchar_t *)malloc(2 * v5 + 2);
  v7 = strlen(a2);
  v8 = (const wchar_t *)malloc(2 * v7 + 2);
  v9 = *(unsigned __int8 *)a1;
  v10 = 0i64;
  v11 = (wchar_t *)v8;
  if ( (_BYTE)v9 )
  {
    do
    {
      v6[v10++] = v9;
      v9 = (unsigned __int8)a1[v10];
    }
    while ( (_BYTE)v9 );
    v10 *= 2i64;
  }
  v12 = *(unsigned __int8 *)a2;
  *(wchar_t *)((char *)v6 + v10) = 0;
  v13 = 0i64;
  if ( (_BYTE)v12 )
  {
    do
    {
      v8[v13++] = v12;
      v12 = (unsigned __int8)a2[v13];
    }
    while ( (_BYTE)v12 );
    v13 *= 2i64;
  }
  *(const wchar_t *)((char *)v8 + v13) = 0;
  sub_66A33C00(v6, v8, a3);
  free(v6);
  free(v11);
}

// ======================================================
// Function: sub_66A33C00 @ 0x66A33C00
// ======================================================

int __fastcall sub_66A33C00(const wchar_t *a1, const wchar_t *a2, int a3)
{
  wchar_t *v6; // rbp
  int result; // eax
  FILE *v8; // rax
  WCHAR Filename[284]; // [rsp+40h] [rbp-238h] BYREF

  v6 = (wchar_t *)malloc(0x4000ui64);
  if ( a2 )
  {
    if ( !*a2 )
      a2 = L"<unknown>";
  }
  else
  {
    a2 = L"<unknown>";
  }
  if ( a1 )
  {
    if ( !*a1 )
      a1 = L"?";
  }
  else
  {
    a1 = L"?";
  }
  if ( !GetModuleFileNameW(0i64, Filename, 0x104u) )
    wcscpy(Filename, L"<unknown>");
  snwprintf(
    v6,
    0x1FFFui64,
    L"Assertion failed!\n\nProgram: %ws\nFile: %ws, Line %u\n\nExpression: %ws",
    Filename,
    a2,
    a3,
    a1);
  if ( !unk_66A3D5B0 )
  {
    v8 = _iob_func();
    fwprintf(v8 + 2, L"%ws\n", v6);
    abort();
  }
  result = MessageBoxW(0i64, v6, L"MinGW Runtime Assertion", 0x12012u);
  if ( result == 3 )
  {
    raise(22);
    exit(3);
  }
  if ( result != 5 )
    abort();
  return result;
}

