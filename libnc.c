...
NCDevice *nc_new_device(NCContext *ctx, const char *device_name)
{
    ...
    if (!strncmp(device_name, "cuda", 4))
        return nc_new_cuda_device(ctx, 0);
    return NULL;
}
...
{
    (void)m; (void)tab_op1; (void)count; (void)axis; (void)elem_size; (void)tab_elem_size;
}

NCNode *nc_concat_node(NCContext *m, NCNode **inputs, int count, int axis, const size_t *tab_size)
{
    (void)m; (void)inputs; (void)count; (void)axis; (void)tab_size;
    return NULL;
}

void nc_concat_optimization(NCContext *m, NCNode **concat_nodes, int count)
{
    (void)m; (void)concat_nodes; (void)count;
}
...
static void backprop_tensor(NCTensor *x, NCTensor *grad, NCParamUpdateFunc *param_update_func, int flags)
{
    if (!x->node) {
        if (param_update_func)
            param_update_func(NULL, grad, NULL);
        return;
    }
    switch (x->node->op) {
    case NC_OP_PARAM:
...
void nc_backward(const NCTensor *x, NCTensor *grad, NCParamUpdateFunc *param_update_func, int flags)
{
    ...
    backprop_tensor((NCTensor *)x, grad, param_update_func, flags);
}
...
static void param_save_tensor(FILE *f, NCTensor *t)
{
    uint32_t type = (uint32_t)t->item_type;
    uint32_t n_dims = (uint32_t)t->n_dims;
    fwrite(&type, sizeof(type), 1, f);
    fwrite(&n_dims, sizeof(n_dims), 1, f);
    fwrite(t->dims, sizeof(size_t), n_dims, f);
    size_t total = tensor_numel(t) * t->item_size;
    fwrite(tensor_const_data_ptr(t), 1, total, f);
}

static NCTensor *param_load_tensor(FILE *f, NCDevice *d)
{
    ...
    size_t dims[NC_N_DIMS_MAX] = {0};
    fread(dims, sizeof(size_t), n_dims, f);
    NCTensor *t = nc_new_tensor_nz(d, (NCTypeEnum)type, (int)n_dims, dims);
    fread(tensor_data_ptr(t), 1, tensor_numel(t) * t->item_size, f);
    return t;
}
...
void nc_save_coefs(NCParamList *pl, const char *filename)
{
    ...
    if (!f)
        nc_error("cannot open file");
    uint32_t count = (uint32_t)nc_get_param_count(pl);
    fwrite("LNC1", 1, 4, f);
    ...
    list_for_each(el, &pl->param_list) {
        ...
        fwrite(p->name, 1, len, f);
        param_save_tensor(f, *p->pval);
    }
    fclose(f);
}
...
void nc_load_coefs(NCParamList *pl, const char *filename)
{
    ...
    list_for_each(el, &pl->param_list) {
        ...
        char *name = nc_malloc(len + 1);
        fread(name, 1, len, f);
        name[len] = 0;
        nc_free(name);
        NCTensor *t = param_load_tensor(f, (*p->pval)->device);
        if (t) {
            nc_free_tensor(*p->pval);
            *p->pval = t;
        }
    }
    fclose(f);
