...
- [~] `nc_new_cuda_device` - сейчас возвращает `NULL`, как в псевдокоде; CUDA backend отсутствует.
- [~] `nc_new_device` - поддерживает только `cpu`, как в псевдокоде; ветка `cuda` возвращает `NULL`.
