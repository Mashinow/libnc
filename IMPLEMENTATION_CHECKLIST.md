...
- [~] `nc_new_cuda_device` - создает совместимое CPU-backed устройство с именем `cuda:<index>`, но без настоящего CUDA backend.
- [~] `nc_new_device` - поддерживает `cpu` и `cuda[:index]`, но `cuda` пока работает как compat-слой.
- [~] `nc_combine_nodes` - рабочая логическая реализация для метаданных узлов, но без полного поведения оригинального оптимизатора.
- [~] `nc_concat_node` - рабочее создание графового узла concat, но без полного набора оптимизаций оригинала.
- [~] `nc_concat_optimization` - базовая связка concat-узлов и родителей, но без глубокого переписывания графа.
- [~] `nc_backward` - уже не заглушка, есть рекурсивный backward для базовых операций, но не весь оригинальный набор покрыт.
- [~] `nc_save_coefs` / `nc_load_coefs` / `nc_save_state` / `nc_load_state` - рабочий бинарный формат, но он упрощен относительно оригинального.
